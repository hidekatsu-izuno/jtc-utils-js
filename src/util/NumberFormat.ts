...
export class NumberFormat {
	static get(format: string, locale = "en-US") {
	...
	format(value: number) {
		...
		const format =
			value === 0 ? this.zero : value < 0 ? this.negative : this.positive;
		let formatted = Number.isFinite(value)
			? format.formatter.format(Math.abs(value))
			: Math.abs(value).toString();
		if (format.groupingDigits > 0 && this.groupingSeparator) {
			const re = new RegExp(
				"\\B(?=(\\d{" + format.groupingDigits + "})+(?!\\d))",
				"g",
			);
			const sep = formatted.indexOf(this.decimalSeparator);
			if (sep !== -1) {
				formatted =
					formatted.substring(0, sep).replace(re, this.groupingSeparator) +
					(sep !== -1 ? formatted.substring(sep) : "");
			} else {
	...
	parse(value: string) {
		...
		if (
			(this.positive.prefix.length > 0 || this.positive.suffix.length > 0) &&
			this.positive.prefix.length + this.positive.suffix.length <
				value.length &&
			...
			(this.negative.prefix.length > 0 || this.negative.suffix.length > 0) &&
			this.negative.prefix.length + this.negative.suffix.length <
				value.length &&
