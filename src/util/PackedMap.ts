...
export class PackedMap {
  ...
  constructor(init: (setter: PackedMapSetter) => void | Map<number, Uint8Array | Uint16Array | Uint32Array>) {
  ...
  get(key: number): number | undefined {
    ...
    if (array) {
      if (array.BYTES_PER_ELEMENT === 1) {
        const shifted = (array[0] << 24 | array[1] << 16 | array[2] << 8 | array[3]) >>> (31 - key2)
        if ((shifted & 0x1) !== 0) {
