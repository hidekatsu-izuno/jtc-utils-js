...
describe("formatDate", () => {
	...
	test("test parse from string", () => {
		expect(parseDate("2000/01/01", "uuuu/M/d")).toStrictEqual(
			new Date(2000, 0, 1),
		);
	});
	...
	test("test parse from string with tz", () => {
		...
		).toStrictEqual(new Date(2000, 0, 1, 0, 0));
		expect(
			parseDate("2000/01/01 00:00:00", "uuuu/M/d H:m:s", { timeZone: "UTC" }),
		).toStrictEqual(new Date(2000, 0, 1, 9));
		expect(
		...
		).toStrictEqual(new Date(2000, 0, 1, 0));
		expect(
	...
	test("test parse japanese calendar", () => {
		...
		).toStrictEqual(new Date(2000, 0, 1));
	});
});
